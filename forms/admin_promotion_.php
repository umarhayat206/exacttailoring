...
if($_POST['addEditPromotionFormSubmitted']=='true'){
	...
	$currentPage->promotionGetfreeItem = str_replace(",", "", $_POST['promotionGetfreeItem']);
...
function productList($productId, $checkedId, $type){			//show fertures list
	$htmlToReturn = "";
	...
	if(!empty($productId)){
		...
		foreach ($productId as $item){
			...
			$htmlToReturn .= " id='$prName' value='1' />";
	...
	<div class="gadget">
		...
		<div class="gadgetblock">
			<form id="contentAddEdit" action="<?=$site_admin; ?>admin_promotion" method="post" enctype="multipart/form-data">
				...
				?>
				<span style="float:right;">GET FREE ITEM</span>
				...
				<input type="radio" id="selectproduct" name="selectitem" value="2" <?php echo $itemtypechk2; ?> style="width:30px; float:right;" />
					...
					<fieldset>
						...
						foreach($category as $catelist){
							...
							if($catelist->categoryRootId>0){
								...
								$productonpromotion = smProduct::searchProduct(" AND productCategoryId = '{$catelist->categoryId}' ", 200, "");
								
								echo productList($productonpromotion, $checkedIndex, 1);	// for on promotion
					...
					<fieldset>
						...
						foreach($category as $catelist){
							...
							if($catelist->categoryRootId>0){
								//echo("<span class='indent'>&nbsp</span>");
								echo "<label>{$catelist->categoryName}</label><br style='margin-bottom:5px;' />";
								$productonpromotion2 = smProduct::searchProduct(" AND productCategoryId = '{$catelist->categoryId}' ", 200, "");
								
								echo productList($productonpromotion2, $checkedIndex2, 2);	// for free item
