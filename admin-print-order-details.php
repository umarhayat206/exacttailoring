...
<div style="width:620px; margin:0 auto;">
	<?php /* <p>
		...
		P O Box 3370, Stratford on Avon CV37 7XR.<br/>
	...
	<p>
		...
		<span style="margin-left:30px;" class="noprint">Order Date: <?php echo $D . " " . $MM . " " . $Y; ?></span><br class="noprint" />
		<span class="noprint"><?php echo $memberDetails->usAddress . " " . $memberDetails->usPostcode . " " . $shocountry; ?></span><br class="noprint" />
		<label class="noprint">Tel: <span style="color:#FF0000;"><?php echo $memberDetails->usTelephone; ?></span></label>
		<label style="margin-left:30px;" class="noprint">Mobile Phone: <span style="color:#FF0000;" class="noprint"><?php echo $memberDetails->usMobile; ?></span></label>
		<label style="margin-left:30px;" class="noprint">Email: <span style="color:#FF0000;"><?php echo $memberDetails->usEmail; ?></span></label>
	</p>
	...
	foreach ($listItems as $item) {
		...
		if (empty($item->itemOnPromotion) || $item->itemOnPromotion == 0) {
			...
			" . $itemDetails;
				...
				if (!empty($item->itemInsideCollarCuff)) {
					echo "<span>Inside Collar &amp; Cuff: {$item->itemInsideCollarCuff}</span><br/>";
				}
			}
			...
			if ($rootname->categoryName !== "Accessories") {
				...
				if ($item->productId != "470") {
					...
					if ($item->itemMeasurementType == 1) {  // Shirt
						...
						echo "<span>Length - {$item->itemmeasurementShirtLenght} $MeasurementMetric</span><br/>
						...
						<span>Neck - {$item->itemmeasurementShirtNeck} $MeasurementMetric</span><br/>
			...
			if ($productitem->productId == "470") {
				...
				echo "<p>" . $productitem->productRefCode . ":: " . $productitem->productTitle . " $forLadies :: Quantity " . $item->itemQty . "<span style='color:#FF0000;'>(this item get FREE - Design same main shirt)</span><br/>Special Details - " . stripcslashes($item->itemSpecialDetails) . "</p><hr/>";
			}
